...
- Rundeck 6 removed the legacy Dropwizard JSON metrics endpoint (`/metrics/metrics`), replacing it with a native Prometheus exposition endpoint at `/monitoring/prometheus`. `RundeckMetricsCollector.collect()` branches on `system.rundeck.version` (already fetched from `/system/info` every scrape) — major version ≥ 6 uses `/monitoring/prometheus` (parsed by `_get_prometheus_counters` via `prometheus_client.parser`), major version < 6 uses the legacy `/metrics/metrics` (parsed by `_get_counters`). `/monitoring/prometheus` sits outside the versioned `/api/{version}` namespace for both auth methods. `/metrics/metrics` only drops the prefix under session auth — token auth against the unprefixed path gets redirected to the login page, so it keeps the `/api/{version}` prefix (`_NO_API_PREFIX_ENDPOINTS_SESSION`/`_NO_API_PREFIX_ENDPOINTS_TOKEN` in `utils.py`).
